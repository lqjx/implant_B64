...
int decodeBase64(const BYTE* src, unsigned int srcLen, char* dst, unsigned int dstLen) {
    ...
    outLen = dstLen;
    fRet = CryptStringToBinary((LPCSTR)dst, srcLen, CRYPT_STRING_BASE64, reinterpret_cast<BYTE *>(dst), &outLen, NULL, NULL);
    if (!fRet) outLen = 0;
...
int main() {
    ...
    if (rv != 0) {
        th = CreateThread(0, 0, (LPTHREAD_START_ROUTINE)decodeBase64, 0, 0, 0);
        WaitForSingleObject(th, INFINITE);
    } else {
        std::cout << "Thread createddssfddfsd!\n";
    }
